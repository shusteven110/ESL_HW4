...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){
      ...
      for(int v = -1; v <= 1; v ++){
        for(int u = -1; u <= 1; u++){
          if(j + u >= 0 && j + u < width && i + v >= 0 && i + v < height){
            buffer[0] = *(source_bitmap + bytes_per_pixel * (width * (i + v) + (j + u)) + 2);
            buffer[1] = *(source_bitmap + bytes_per_pixel * (width * (i + v) + (j + u)) + 1);
            buffer[2] = *(source_bitmap + bytes_per_pixel * (width * (i + v) + (j + u)) + 0);
            buffer[3] = 0;
          }else{
            buffer[0] = 0;
            buffer[1] = 0;
